...
export default function StationPickerModal({ onClose, onSelect, title }) {
  ...
  const filtered = useMemo(() => {
    if (!query) return [];
    return allStationsList.filter(s => s.stationName.includes(query) || s.lineLabel.includes(query));
  }, [query]);
                    ...
                    {data.stations.map((st) => (
                      ...
                      >
                        ...
                        <div style={{ width: '48px', display: 'flex', justifyContent: 'center' }}>
                          <div style={{ width: '16px', height: '16px', borderRadius: '50%', backgroundColor: 'white', border: `4px solid ${data.color}`, zIndex: 2 }}></div>
                        </div>
